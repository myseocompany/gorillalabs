...
<div class="max-w-7xl mx-auto bg-white p-8 rounded-lg shadow-md">
    ...
    <div class="grid grid-cols-2 gap-6 mt-6">
        <div>
            ...
            <p class="text-gray-900 p-2 bg-gray-200 rounded-md">{{ $lab->accreditation_onac ? 'Sí' : 'No' }}</p>
        </div>

        <div>
            <label class="block font-medium text-gray-700">Acreditación IDEAM</label>
            <p class="text-gray-900 p-2 bg-gray-200 rounded-md">{{ $lab->accreditation_ideam ? 'Sí' : 'No' }}</p>
        </div>
    </div>

    <div class="mt-6 flex justify-end space-x-4">
       
        <a href="{{ url('/') }}" class="bg-gray-500 text-white px-6 py-2 rounded-md hover:bg-gray-600 transition">
            Volver al Inicio
        </a>
        <a href="{{ route('labs.edit') }}" class="bg-blue-500 text-white px-6 py-2 rounded-md hover:bg-blue-600 transition">
            Editar
        </a>
